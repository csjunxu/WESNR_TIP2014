...
idx      =   s_idx(seg(1)+1:seg(2));    %得到第一类  6701*1 5381*1 5380*1 5326*1 5330*1 5326*1 5472*1
set      =   1:size(X_m,2);
set(idx) =   [];

%for i = 1:par.nblk
 %  v            =  Wei(i,set);     %355 1675 1676 1730 1726 1730 1584
 %  X_m(:,set)   =  X_m(:,set) + X(:, Arr(i, set)) .*v(ones(b2,1), :);    %非平滑块b1x1+b2x2+...b15x15
%end
...
for   i  = 2:length(seg)-1
    ...
    idx  =  s_idx(seg(i)+1:seg(i+1));   %(seg(2)+1):seg(3)得到指标为次小的类
    X_m  =  mean(X(:, idx),2);
    X_m  =  repmat(X_m,1,size(X(:, idx),2));
    cls  =  PCA_idx(idx(1));
    P    =   reshape(PCA_D(:, cls), b2, b2);   %将字典相应的类重够为49*49
    ...
    Y(:, idx)    =   P'*soft(P*(X(:, idx)-X_m), tau) + X_m;    %49*7056稀疏表示
end
